...
def saved(servers, **extra):
    config.CONFIG_PATH.write_text(
        json.dumps({"version": 1, "enabled": True, "servers": servers, **extra}),
        encoding="utf-8",
    )
...
def command(value=None):
    return {
        ...
        "expected_revision": "0",
        "payload": {
            "configuration_revision": controls.read_mcp_configuration().revision,
...
def execute(value, **options):
    return controls.execute_mcp_configuration_command(
        owner_id="synthetic-owner",
        key=value["command_id"],
        command=value,
        validate=options.get("validate", lambda: None),
        validate_review=options.get("validate_review", lambda _: None),
    )


def test_original_receipt_rehashes_current_bytes_after_saved_snapshot(
    owner, monkeypatch
):
...
def test_saved_disabled_revokes_actual_retained_tool_before_schedule(
    ...
    monkeypatch.setattr(
        ...
        {
            "Synthetic": {
                "read": runtime.McpToolInfo(
                    "Synthetic", "read", "mcp_synthetic_read", enabled=True
                )
            }
        },
    )
    monkeypatch.setattr(
        runtime,
        "_servers",
        {"Synthetic": runtime.McpServerRuntime("Synthetic", {"tool_timeout": 1})},
    )
    bound = runtime._make_tool_func("Synthetic", "read", enforce_policy=True)
    monkeypatch.setattr(
        runtime, "_schedule", lambda *_a, **_k: pytest.fail("Revoked tool scheduled")
    )
    identifier = controls.read_mcp_configuration().items[0].server_id
    fields = (
        {"name": "Renamed"}
        if operation == "rename"
        else {"args": ["one two", "--exact"]}
    )
    result = execute(
        command({"operation": operation, "server_id": identifier, "fields": fields})
    )
...
def test_import_collision_is_atomic_and_valid_import_preserves_unknowns(owner):
    ...
    with pytest.raises(
        controls.CapabilityConfigurationError, match="mcp_server_collision"
    ):
        execute(
            command(
                {
...
def test_requirement_summary_is_passive_bounded_and_path_free(owner, monkeypatch):
    ...
    calls = []
    check = requirements.RuntimeCheck(
        requirement=requirements.RuntimeRequirement(
            "node", "Node.js LTS", ("node",), managed=True
        ),
        available=False,
        paths={"node": "C:\\private\\node.exe"},
        message="private-token",
    )
    monkeypatch.setattr(
        requirements,
        "check_server_requirements",
        lambda server: calls.append(server) or [check],
    )
    monkeypatch.setattr(
        requirements,
        "install_managed_runtime",
        lambda *_: pytest.fail("read installed runtime"),
    )
    before = config.CONFIG_PATH.read_bytes()
    page = controls.read_mcp_configuration()
    assert page.items[0].requirements == (
        {
            "id": "node",
            "label": "Node.js LTS",
            "available": False,
    ...
    assert "private-token" not in json.dumps(asdict(page))
    assert "C:\\private" not in json.dumps(asdict(page))
    assert calls and config.CONFIG_PATH.read_bytes() == before


def test_deleted_server_uses_exact_revision_and_original_cleanup_receipt(
    owner, monkeypatch
):
    from row_bot.mcp_client import runtime

    saved({"Synthetic": {"enabled": True, "command": "synthetic-command"}})
    server_id = controls.read_mcp_configuration().items[0].server_id
    deletion = {"operation": "delete", "server_id": server_id}
    value = command(deletion)
    calls = []
    monkeypatch.setattr(
        runtime,
        "get_server_lifecycle",
        lambda name: {"runtime_id": "synthetic-runtime"},
    )
    monkeypatch.setattr(
        runtime,
        "stop_server_owned",
        lambda name, runtime_id: (
            calls.append((name, runtime_id))
            or {"state": "cleanup_incomplete" if len(calls) == 1 else "stopped"}
        ),
    )
    first = execute(value)
    assert first["status"] == "partial"
    assert first["mcp_configuration"]["code"] == "mcp_cleanup_incomplete"
    assert "Synthetic" not in config.read_saved_configuration().document["servers"]
    data = config.CONFIG_PATH.read_bytes()
    second = execute(value)
    assert second["status"] == "completed"
    assert second["mcp_configuration"]["runtime_cleanup"] == "stopped"
    assert calls == [("Synthetic", "synthetic-runtime")] * 2
    assert config.CONFIG_PATH.read_bytes() == data
    assert execute(value) == second
    with pytest.raises(
        controls.CapabilityConfigurationError, match="revision_conflict"
    ):
        controls.review_mcp_configuration_command(
            value["payload"]["configuration_revision"], deletion, validate=lambda: None
        )
...
def test_response_loss_reconciles_owned_publication_without_new_write(
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: pytest.fail("No publication replay"),
    )
    result = execute(
        value,
        validate_review=lambda _: pytest.fail(
            "Read-only reconciliation needs no new effect approval"
        ),
    )
    assert result["status"] == "completed" and "_mcp_configuration" not in result
    assert (
        config.CONFIG_PATH.read_bytes() == current
        and config.CONFIG_PATH.stat().st_ino == inode
    )
...
def test_equal_bytes_replacement_never_proves_uncertain_publication(owner, monkeypatch):
    ...
    monkeypatch.setattr(
        config,
        "publish_saved_configuration",
        lambda *_a, **_k: pytest.fail("No blind retry"),
    )
    result = execute(value)
    assert (
        result["status"] == "partial"
        and result["mcp_configuration"]["saved_disabled"] is None
    )
    assert config.CONFIG_PATH.read_bytes() == data
...
def test_failed_publication_retains_durable_proof_and_blocks_missing_recreation(
    ...
    assert (
        len(rows["items"]) == 1
        and rows["items"][0]["type"] == "mcp.configuration.legacy_save"
    )
    row = rows["items"][0]
...
def test_unknown_pending_overflow_and_query_failure_never_means_safe_empty(
    owner, monkeypatch
):
    from row_bot.runtime import admissions
    ...
    monkeypatch.setattr(
        admissions,
        "read_unfinished_target_commands",
        lambda *_a, **_k: {"items": [], "overflow": True},
    )
    assert controls.read_mcp_configuration().availability == "recovery_required"
    with pytest.raises(config.McpConfigurationError, match="recovery_required"):
        config.save_config({})
...
def test_invalid_typed_fields_never_reach_publication(owner, monkeypatch, fields):
    monkeypatch.setattr(
        config,
        "publish_saved_configuration",
        lambda *_a, **_k: pytest.fail("Invalid input publication"),
    )
...
def test_config_wire_and_page_budgets_remain_distinct(owner):
    ...
    assert (
        len(
            config.read_saved_configuration().document["servers"]["Original"][
                "unknown_future"
            ]
        )
        == 1024 * 1024 + 1
    ...
    saved(
        {
            f"Synthetic {i:05}": {"command": "synthetic", "env": {"VALUE": "x" * 32}}
            for i in range(205)
        }
    )
    assert (
        len(json.dumps(asdict(controls.read_mcp_configuration(limit=50))).encode())
        < 256 * 1024
    )
...
def test_malformed_loaded_status_is_bounded_unknown(owner, monkeypatch):
    ...
    monkeypatch.setattr(
        runtime,
        "_statuses",
        {"Synthetic": SimpleNamespace(status=["connected"], tool_count=2**128)},
    )
...
def test_nonce_revocation_after_durable_checkpoint_prevents_retirement(
    owner, monkeypatch
):
    from row_bot.runtime import admissions
...
def test_concurrent_legacy_save_forces_new_client_revision_review(owner, monkeypatch):
    ...
    saved(
        {"Original": {"command": "synthetic", "enabled": True}}, future={"keep": True}
    )
    value = command()
    ...
    assert "Synthetic" not in document["servers"] and document["future"] == {
        "keep": True
    }
...
def test_saved_status_redacts_launch_values_and_never_refreshes_runtime(
    owner, monkeypatch
):
    from row_bot.mcp_client import runtime
    ...
    saved(
        {
            "Synthetic": {
                ...
                "cwd": str(owner / "private-path-value"),
                "env": {"ORDINARY": "private-env-value"},
                "headers": {"Ordinary": "private-header-value"},
                "url": "https://private-url.invalid/token",
                "tools": {
                    "catalog": {"read": {"description": "private-description-value"}}
                },
            }
        }
    )
    ...
    assert set(page.items[0].configured_fields) == {
        "command",
        "args",
        "cwd",
        "url",
...
def test_full_library_pages_and_filter_bind_cursor_to_current_snapshot(owner):
    saved(
        {
            f"Server {index:04}": {"command": "synthetic", "enabled": False}
    ...
    with pytest.raises(controls.CapabilityConfigurationError, match="cursor_expired"):
        controls.read_mcp_configuration(
            query="different", limit=50, cursor=first.next_cursor
        )
...
def test_malformed_saved_labels_and_toggles_are_not_authority(owner):
    saved(
        {
            "../private-path": {"enabled": "true", "command": "synthetic"},
...
def test_explicit_large_config_publication_keeps_default_editor_budget(owner):
    ...
    with pytest.raises(edits.FileEditError, match="file_too_large"):
        edits.publish_text_revision(
            owner,
            path.name,
            data,
            expected_digest="missing",
            command_id=str(uuid4()),
            persist_recovery=proofs.append,
        )
    assert not path.exists() and proofs == []
    result = edits.publish_text_revision(
        owner,
        path.name,
        data,
        expected_digest="missing",
    ...
    assert (
        result.digest == hashlib.sha256(data.encode()).hexdigest()
        and path.read_bytes() == data.encode()
    )
    with pytest.raises(edits.FileEditError, match="file_too_large"):
        edits.read_edit_bytes(owner, path.name)
    assert (
        edits.read_edit_bytes(owner, path.name, max_bytes=8 * 1024 * 1024)[0]
        == data.encode()
    )
...
def test_publication_budget_invalid_before_effect(owner, limit):
    ...
    with pytest.raises(edits.FileEditError, match="invalid_edit"):
        edits.publish_text_revision(
            ...
            expected_digest="missing",
            command_id=str(uuid4()),
            persist_recovery=lambda _: pytest.fail("No receipt"),
            max_bytes=limit,
        )
...
def test_save_disabled_creates_exact_argv_once_and_omitted_edit_values_are_retained(
    ...
    value = command(
        intent(
            args=["two words", "", "literal'quote"],
            env={"ORDINARY": "synthetic-secret"},
        )
    )
    result = execute(value)
    ...
    assert (
        server["args"] == ["two words", "", "literal'quote"]
        and server["enabled"] is False
    )
    identity = controls.read_mcp_configuration().items[0].server_id
    execute(
        command(
            {"operation": "edit", "server_id": identity, "fields": {"tool_timeout": 35}}
        )
    )
