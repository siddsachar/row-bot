...
def execute(command, review, **overrides):
    ...
    return commands.execute_skill_command(
        command,
        owner_id=overrides.pop("owner_id", "owner"),
        authority_id=overrides.pop("authority_id", "authority"),
        key=command["command_id"],
        validate=overrides.pop("validate", noop),
        validate_action=overrides.pop("validate_action", noop),
        validate_review=overrides.pop("validate_review", accepted),
        **overrides,
    )
...
def fields(**updates):
    return {
        "display_name": "Created skill",
        "icon": "✨",
        "description": "A bounded skill.",
        "instructions": "Use only the explicitly approved inputs.",
        "tags": ["saved"],
        "activation": {"keywords": ["approved"]},
        "version": "1.0",
        **updates,
    }


def test_passive_snapshot_uses_current_bytes_without_loading_or_writing(
    library, monkeypatch
):
    skills = library
    monkeypatch.setattr(
        skills, "load_skills", lambda: pytest.fail("passive skill migration")
    )
    before = sorted(str(p) for p in skills.DATA_DIR.rglob("*"))
    snapshot = skills.read_client_skills()
    assert (
        snapshot["items"]["sample"]["skill"].instructions == "Use the approved tools."
    )
...
def test_global_pin_publishes_single_link_and_availability_removes_pin(library):
    ...
    snapshot = skills.read_client_skills()
    receipts = []
    skills.update_client_skill_preference(
        "sample",
        "pin_defaults",
        True,
...
def test_linked_files_are_not_adopted_or_modified(library, tmp_path, target):
    ...
    path = (
        skills.CONFIG_PATH
        if target == "config"
        else skills.USER_SKILLS_DIR / "sample" / "SKILL.md"
    )
    if path.exists():
...
def test_shared_retirement_preserves_original_and_confirms_exact_proof(tmp_path):
    ...
    assert (
        tmp_path / ".row-bot-edit-recovery" / proof[0].command_id / "previous"
    ).read_bytes() == before
...
def test_client_library_and_detail_are_bounded_path_free_and_passive(
    library, monkeypatch
):
    monkeypatch.setattr(
        library, "load_skills", lambda: pytest.fail("passive read loaded skills")
    )
    before = {
        str(path): path.read_bytes()
        for path in library.DATA_DIR.rglob("*")
        if path.is_file()
    }
    ...
    assert page["items"][0] == {
        ...
        "instructions_preview": "Use the approved tools.",
        "truncated": False,
    }
...
def test_client_library_filters_and_sorts_before_paging(library, monkeypatch):
    for name, display, text in [
        ("second", "Zebra", "short"),
        ("third", "Aardvark", "Long instructions. " * 20),
    ]:
        target = library.USER_SKILLS_DIR / name
        target.mkdir()
    ...
    monkeypatch.setattr(provenance, "load_records", lambda: {"third": object()})
    monkeypatch.setattr(
        "row_bot.skills_activation.get_skill_telemetry",
        lambda: {
            "second": {"last_used": "2025-01-02T00:00:00Z"},
            "third": {"last_used": "2025-01-01T00:00:00Z"},
        },
    )
    public = commands.read_skill_library(filter="public", validate=noop)
    assert [item["id"] for item in public["items"]] == ["third"]
    assert public["items"][0]["public"] is True
    assert [
        item["id"]
        for item in commands.read_skill_library(sort="recent", validate=noop)["items"]
    ][:2] == ["second", "third"]
    assert (
        commands.read_skill_library(sort="tokens", validate=noop)["items"][0]["id"]
        == "second"
    )
    first = commands.read_skill_library(sort="tokens", limit=1, validate=noop)
    next_page = commands.read_skill_library(
        sort="tokens", limit=1, cursor=first["next_cursor"], validate=noop
    )
    assert next_page["items"][0]["id"] != first["items"][0]["id"]
    with pytest.raises(Exception, match="cursor_expired"):
        commands.read_skill_library(
            sort="source", limit=1, cursor=first["next_cursor"], validate=noop
        )
...
def test_reviewed_create_edit_duplicate_delete_and_exact_recovery(library, monkeypatch):
    ...
    assert "Use only the explicitly approved inputs." in created_path.read_text(
        encoding="utf-8"
    )
    ...
    assert "Edited only after explicit review." in created_path.read_text(
        encoding="utf-8"
    )

    snapshot = library.read_client_skills()
    duplicate, duplicate_review = prepare(
        "skill.duplicate",
        {
            "revision": snapshot["revision"],
            "name": "created",
            "new_name": "created_copy",
        },
    )
    ...
    monkeypatch.setattr(
        admissions,
        "complete_command",
        lambda *_args: (_ for _ in ()).throw(OSError("lost receipt")),
    )
...
def test_import_canonicalizes_manual_skill_and_rejects_tool_guides(library):
    ...
    command, review = prepare(
        "skill.import", {"revision": snapshot["revision"], "content": content}
    )
    result = execute(command, review)
    assert result["skill_id"] == "imported"
    saved = (library.USER_SKILLS_DIR / "imported" / "SKILL.md").read_text(
        encoding="utf-8"
    )
...
@pytest.mark.parametrize(
    "preference,value,available,pinned",
    [
        ("pin_defaults", True, True, True),
        ("availability", False, False, False),
    ],
)
def test_reviewed_preferences_preserve_pin_invariants(
    library, preference, value, available, pinned
):
    if preference == "availability":
        snapshot = library.read_client_skills()
        first, first_review = prepare(
            "skill.preference",
            {
                "revision": snapshot["revision"],
                "name": "sample",
...
def test_changed_source_and_revoked_review_never_start_a_skill_effect(library):
    ...
    denied, denied_review = prepare(
        "skill.edit",
        {
            "revision": fresh["revision"],
            "name": "sample",
            "skill_revision": fresh["items"]["sample"]["revision"],
            "fields": {"description": "Denied"},
        },
    )
    with pytest.raises(ValueError, match="revoked"):
        execute(
            denied,
            denied_review,
            validate_action=lambda *_: (_ for _ in ()).throw(ValueError("revoked")),
        )
...
def test_proposal_reads_are_passive_redacted_and_actions_use_canonical_owner(library):
    ...
    proposal = {
        ...
        "title": "<script>Proposal</script>",
        "rationale": "Review this proposal.",
        "risk": "low",
        "status": "ready",
        "preview": {
            "instructions_preview": r"Plain text C:\Users\private sk-abcdefghijklmnop."
        },
    }
    ...
    assert before == store.read_bytes() and str(library.DATA_DIR) not in json.dumps(
        result
    )
    ...
    command, review = prepare(
        "skill.proposal.reject",
        {
            ...
            "reason": "Does not fit this workflow.",
        },
    )
...
def test_reviewed_proposal_apply_uses_canonical_evolution_and_is_not_replayed(
    library, monkeypatch
):
    ...
    proposal = evolution.create_proposal(
        insight_ids=[],
        proposal_type="create_skill",
        title="Create proposed skill",
        rationale="A repeatable workflow.",
        payload={
            "name": "proposed_skill",
            "display_name": "Proposed skill",
            "icon": "✨",
            "description": "A reviewed proposal.",
            "instructions": "Follow the reviewed workflow.",
            "tags": ["proposal"],
            "enabled": True,
            "version": "1.0",
        },
    ...
    command, review = prepare(
        ...
        {"revision": snapshot["revision"], "proposal_id": proposal["id"], "reason": ""},
    )
    result = execute(command, review)
    assert result["status"] == "completed"
    ...
    monkeypatch.setattr(
        evolution,
        "apply_proposal",
        lambda *_args, **_kwargs: pytest.fail("no proposal replay"),
    )
