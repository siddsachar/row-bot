...
## Unreleased — v3.21.0 Buddy Companion Foundation

- **Buddy companion subsystem** — adds a prompt-generated Buddy architecture with a thread-safe event bus, deterministic behavior brain, persistent config, pack validation, in-app Hatch art and state-motion generation, canvas playback/effects, one dockable in-app Buddy, and a separate desktop overlay surface.
- **Live Thoth awareness** — Buddy now receives chat streaming, thinking, tool, approval, workflow, notification, and voice-state events from the existing runtime paths.
...
- **Generated animation boundary** — Buddy ships with bundled first-party `glyph`, `lumen`, `ember`, `pixel`, `sprout`, and `orbit` generated-motion packs, while Hatch-generated custom Buddy art and compact image-to-video motion packs are copied into Thoth's served Buddy assets; normal Buddy playback switches locally across idle, thinking, working, approval, success, and error states with no runtime model calls, and generated MP4 clips are corner-keyed onto transparent Buddy surfaces for undocked in-app and desktop overlay use. Hatch prompts request keyable backgrounds, frame padding, and rim-lit dark edges so generated packs preserve character detail during transparency compositing, let the user's concept own the theme instead of forcing Thoth-like art direction, Google Veo request starts are paced during fresh bundled regenerations, and the runtime now uses gentler corner-seeded transparency thresholds so Orbit's head and the Pixel, Sprout, and Ember edges stay intact while still removing flat video backgrounds. The runtime also softens approval motion, crossfades state changes, smooths MP4 loop restarts, replays idle motion once a minute so Buddy feels attentive without looking alarmed, and keeps motion semantics explicit so pending approval uses the approval clip, denied/timed-out/cancelled/interrupted states use the error/recovery clip, and only completion events use success clips.
- **Buddy surface polish** — Buddy now has one in-app presence: it starts inside a sidebar home circle, can be dragged into the workspace, leaves the sidebar dock visibly empty while away, snaps home when released near the dock, and returns home on app restart instead of persisting a stray position. The in-app presence acts as the single click target for a quick greeting plus settings, sidebar status text is restored while docked, undocked in-app and desktop surfaces show compact activity bubbles, settings hot-apply in-app/desktop visibility, the Buddy tab now groups where it appears, behavior, look, and generated-motion guidance in a dense Models-tab-style layout, the look picker uses polished visual preview tiles with concise pack names instead of label-only choices, selecting a bundled visual tile clears active and latest Hatch media overrides and refreshes existing in-app and desktop clients so the chosen pack is not masked by a previous generated look or stale overlay window, chatty bubbles use the selected companion personality, Hatch generation includes the generation notes intentionally grouped with the prompt, the native desktop overlay opens from the active pywebview JS API instead of silently failing, uses a transparent frameless focusless window where supported, and Settings exposes only an in-app toggle plus desktop overlay toggle/open controls. Approval denials and timeouts now clear both approval and workflow activity immediately, denied, timed-out, stopped, or cancelled workflow endings clear Buddy's workflow-step state, and successful multi-step workflow endings emit `done` instead of a misleading cancellation.
- **Buddy desktop overlay reliability** — desktop Buddy now preserves important approval, denial, workflow, and error bubbles even in Quiet mode, keeps those bubbles visible briefly across rapid state settling, applies first-paint transparent document styling, creates the primary native overlay hidden, blocks focus-sync show calls until the Buddy page marks itself ready, reveals only after the transparent Buddy document has painted, retries native overlay creation with simpler pywebview options if a backend rejects transparency or hidden-window hints, avoids snapshot pushes into deleted NiceGUI clients, and guards startup health-check results so a transient `None` cannot crash the native window while an approval is being denied.
- **Settings and timer stability** — Settings → Models now opens the provider/model catalog lazily and caps provider rows so very large catalogs no longer crash the UI, while NiceGUI one-shot and polling timers now clean up when clients disconnect or parent slots are deleted instead of flooding logs with deleted-slot errors.
- **Linux launcher install-path fix** — the generated Linux launcher now resolves installed symlink chains before computing the app root, so `~/.local/bin/thoth` correctly starts the packaged app from `~/.local/share/thoth/current`; release CI now smokes through the installed user launcher path.
- **Linux installer UX hardening** — source-checkout builds now support the root-level `bash build_linux_app.sh <version>` support command, Linux install success messages now print `~/.local/bin/thoth` when `~/.local/bin` is not on `PATH`, and maintainer docs now separate unreleased tarball testing from the one-line installer that resolves published GitHub Release assets.
- **Startup diagnostics for optional native packages** — startup now detects installed-but-broken optional native packages such as TorchCodec, logs a concrete recovery command, and makes Transformers treat a broken TorchCodec install as unavailable instead of letting optional audio/video helpers crash Thoth during startup.
- **Windows embedded-Python repair hardening** — Windows installer repair/upgrade now replaces the bundled `{app}\python` runtime before copying the new payload, preventing manually installed or corrupted packages in Thoth's embedded Python from surviving an over-the-top reinstall.
- **Focused regression coverage** — new Buddy tests cover core event/config/asset behavior, Hatch motion activation, UTF-8 config loading, UI wiring, event source hooks, runtime fallback behavior, dockable in-app behavior, built-in motion semantics, and packaging inclusion; manual-style browser smokes verify docked, undocked, and overlay playback from the bundled pack.
- **Focused reliability coverage** — new startup hardening tests cover broken TorchCodec detection, launcher recovery hints, Windows installer embedded-Python replacement, app import smoke, Settings → Models catalog bounds, safe timer cleanup, and installed Linux launcher symlink resolution.
