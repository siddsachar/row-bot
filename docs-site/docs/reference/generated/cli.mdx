...
| --- | --- | --- | --- | --- |
| row-bot | --browser |  |  | `src/row_bot/launcher.py:3723` |
| row-bot | --client-v2 | Deprecated no-op alias for the default React client at /app-v2/ |  | `src/row_bot/launcher.py:3745` |
| row-bot | --host | Host/interface for the Row-Bot local server |  | `src/row_bot/launcher.py:3781` |
| row-bot | --legacy-ui | Open the retained legacy local UI at / |  | `src/row_bot/launcher.py:3750` |
| row-bot | --native |  |  | `src/row_bot/launcher.py:3728` |
| row-bot | --no-ollama | Do not try to auto-start Ollama |  | `src/row_bot/launcher.py:3758` |
| row-bot | --no-open | Do not open a browser or native window |  | `src/row_bot/launcher.py:3742` |
| row-bot | --no-splash | Skip the launcher splash screen |  | `src/row_bot/launcher.py:3755` |
| row-bot | --no-tray | Run without a system tray icon |  | `src/row_bot/launcher.py:3736` |
| row-bot | --port |  | _PORT | `src/row_bot/launcher.py:3778` |
| row-bot | --reset-db | Back up known local SQLite DBs before launch |  | `src/row_bot/launcher.py:3766` |
| row-bot | --reset-tasks-db | Back up and recreate tasks.db before launch |  | `src/row_bot/launcher.py:3761` |
| row-bot | --restore-data | Restore known SQLite DBs from a recovery backup directory or latest backup |  | `src/row_bot/launcher.py:3771` |
| row-bot | --server | Run the server without tray integration |  | `src/row_bot/launcher.py:3739` |
| row-bot | --tray | Force the system tray launcher |  | `src/row_bot/launcher.py:3733` |
...
| row-bot plugin doctor | plugin |  |  | `src/row_bot/launcher.py:3720` |
| row-bot plugin link | path |  |  | `src/row_bot/launcher.py:3712` |
| row-bot plugin reload | plugin_id |  |  | `src/row_bot/launcher.py:3716` |
| row-bot plugin validate | path |  |  | `src/row_bot/launcher.py:3710` |
