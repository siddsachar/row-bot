...
| --- | --- | --- | --- | --- |
| row-bot | --browser |  |  | `src/row_bot/launcher.py:3825` |
| row-bot | --client-v2 | Deprecated no-op alias for the default React client at /app-v2/ |  | `src/row_bot/launcher.py:3847` |
| row-bot | --host | Host/interface for the Row-Bot local server |  | `src/row_bot/launcher.py:3883` |
| row-bot | --legacy-ui | Open the retained legacy local UI at / |  | `src/row_bot/launcher.py:3852` |
| row-bot | --native |  |  | `src/row_bot/launcher.py:3830` |
| row-bot | --no-ollama | Do not try to auto-start Ollama |  | `src/row_bot/launcher.py:3860` |
| row-bot | --no-open | Do not open a browser or native window |  | `src/row_bot/launcher.py:3844` |
| row-bot | --no-splash | Skip the launcher splash screen |  | `src/row_bot/launcher.py:3857` |
| row-bot | --no-tray | Run without a system tray icon |  | `src/row_bot/launcher.py:3838` |
| row-bot | --port |  | _PORT | `src/row_bot/launcher.py:3880` |
| row-bot | --reset-db | Back up known local SQLite DBs before launch |  | `src/row_bot/launcher.py:3868` |
| row-bot | --reset-tasks-db | Back up and recreate tasks.db before launch |  | `src/row_bot/launcher.py:3863` |
| row-bot | --restore-data | Restore known SQLite DBs from a recovery backup directory or latest backup |  | `src/row_bot/launcher.py:3873` |
| row-bot | --server | Run the server without tray integration |  | `src/row_bot/launcher.py:3841` |
| row-bot | --tray | Force the system tray launcher |  | `src/row_bot/launcher.py:3835` |
...
| row-bot plugin doctor | plugin |  |  | `src/row_bot/launcher.py:3822` |
| row-bot plugin link | path |  |  | `src/row_bot/launcher.py:3814` |
| row-bot plugin reload | plugin_id |  |  | `src/row_bot/launcher.py:3818` |
| row-bot plugin validate | path |  |  | `src/row_bot/launcher.py:3812` |
