...
| --- | --- | --- | --- | --- |
| row-bot | --browser |  |  | `src/row_bot/launcher.py:3710` |
| row-bot | --client-v2 | Open the opt-in React client at /app-v2/ (does not change the default client) |  | `src/row_bot/launcher.py:3732` |
| row-bot | --host | Host/interface for the Row-Bot local server |  | `src/row_bot/launcher.py:3763` |
| row-bot | --native |  |  | `src/row_bot/launcher.py:3715` |
| row-bot | --no-ollama | Do not try to auto-start Ollama |  | `src/row_bot/launcher.py:3740` |
| row-bot | --no-open | Do not open a browser or native window |  | `src/row_bot/launcher.py:3729` |
| row-bot | --no-splash | Skip the launcher splash screen |  | `src/row_bot/launcher.py:3737` |
| row-bot | --no-tray | Run without a system tray icon |  | `src/row_bot/launcher.py:3723` |
| row-bot | --port |  | _PORT | `src/row_bot/launcher.py:3760` |
| row-bot | --reset-db | Back up known local SQLite DBs before launch |  | `src/row_bot/launcher.py:3748` |
| row-bot | --reset-tasks-db | Back up and recreate tasks.db before launch |  | `src/row_bot/launcher.py:3743` |
| row-bot | --restore-data | Restore known SQLite DBs from a recovery backup directory or latest backup |  | `src/row_bot/launcher.py:3753` |
| row-bot | --server | Run the server without tray integration |  | `src/row_bot/launcher.py:3726` |
| row-bot | --tray | Force the system tray launcher |  | `src/row_bot/launcher.py:3720` |
...
| row-bot plugin doctor | plugin |  |  | `src/row_bot/launcher.py:3707` |
| row-bot plugin link | path |  |  | `src/row_bot/launcher.py:3699` |
| row-bot plugin reload | plugin_id |  |  | `src/row_bot/launcher.py:3703` |
| row-bot plugin validate | path |  |  | `src/row_bot/launcher.py:3697` |
