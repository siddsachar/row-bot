...
from row_bot.ui.timer_utils import safe_ui_task

from row_bot.designer.references import delete_project_reference, persist_project_references
...
from row_bot.designer.preview import build_preview
from row_bot.designer.page_navigator import build_page_navigator
...
def build_designer_editor(
    ...
    *,
    ...
    def _render_zero_state_quick_actions() -> None:
        ...
        from row_bot.designer.zero_state import get_quick_actions, is_project_empty
    ...
    with ui.row().classes("w-full items-center shrink-0").style(
        ...
        def _go_back():
            ...
            try:
                from row_bot.ui.state import state as _state
        ...
        from row_bot.designer.preview import INTERACTIVE_MODES as _INT_MODES
        ...
        if _project_mode in {"deck", "storyboard"}:
            async def _show_presentation():
                from row_bot.designer.presentation import show_presentation
        ...
        # Brand — all modes
        def _show_brand():
            from row_bot.designer.brand_dialog import show_brand_dialog
        ...
        if _project_mode in {"deck", "landing"}:
            def _show_blocks():
                from row_bot.designer.components import list_components, render_component_html
        ...
        if _project_mode in {"deck", "document"}:
            def _show_import():
                from row_bot.designer.import_dialog import show_import_dialog
        ...
        def _open_review() -> None:
            from row_bot.designer.review_dialog import open_review_dialog
        ...
        def _open_command_palette() -> None:
            from row_bot.designer.command_palette import open_command_palette
        ...
        # Export button
        def _show_export():
            from row_bot.designer.export_dialog import show_export_dialog
            show_export_dialog(project)

        def _show_share():
            from row_bot.designer.share_dialog import show_share_dialog
...
def _open_history_dialog(
    ...
    from row_bot.designer.history import list_snapshots, restore_snapshot
