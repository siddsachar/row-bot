...
def build_graph_panel() -> None:
    ...
    from row_bot.docs_capture import marketing_capture_knowledge_ids
    ...
    with timed_ui_section("graph_panel.data", threshold_ms=1000, max_nodes=250):
        capture_ids = marketing_capture_knowledge_ids()
        if capture_ids:
            wanted = set(capture_ids)
            nodes_by_id = {}
            edges_by_id = {}
            for entity_id in capture_ids:
                item = kg.graph_to_vis_json(
                    entity_id=entity_id,
                    hops=1,
                    max_nodes=len(capture_ids),
                )
                ...
                for edge in item.get("edges", []):
                    if (
                        str(edge.get("from") or "") in wanted
                        and str(edge.get("to") or "") in wanted
                    ):
                        edges_by_id[str(edge.get("id") or "")] = edge
            capture_nodes = [
                nodes_by_id[entity_id]
                for entity_id in capture_ids
                if entity_id in nodes_by_id
            ]
            capture_edges = list(edges_by_id.values())
            data = {
                "nodes": capture_nodes,
                "edges": capture_edges,
                "center": capture_nodes[0]["id"] if capture_nodes else None,
                "stats": {
                    "total_entities": len(capture_nodes),
                    "total_relations": len(capture_edges),
                    "shown_nodes": len(capture_nodes),
                    "shown_edges": len(capture_edges),
                },
            }
        else:
    ...
    stats = data["stats"]
    payload_chars = len(
        _json.dumps({"nodes": data.get("nodes", []), "edges": data.get("edges", [])})
    )
    log_ui_perf(
        "graph_panel.render",
    ...
    if stats["total_entities"] == 0:
        with ui.column().classes("w-full h-full items-center justify-center"):
            ui.icon("hub").classes("text-grey-6").style(
    ...
    with (
        ui.row()
        .classes("w-full items-center gap-2 q-px-sm q-py-xs shrink-0")
        ...
        btn_dream = (
            ui.button("🌙 Dream", on_click=_run_dream_now)
